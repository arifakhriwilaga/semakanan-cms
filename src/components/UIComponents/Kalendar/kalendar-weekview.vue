    ...
    <div class="sticky-top">
      ...
      <ul class="days">
        <li class="day-indicator" :key="index" v-for="({date}, index) in (days || [])" :class="{'today': _isToday(date), 'is-before': isBefore(date)}">
          <span class="letters-date">{{ formatIDDate(date, 'dddd') }}</span>
        </li>
      </ul>
      <ul class="all-day">
        <span>All Day</span>
...
export default {
  ...
  methods: {
    formatIDDate(_format, how) {
      for (let index = 0; index < this.formatLocal.length; index++) {
        if(this.formatLocal[index].english == format(_format, how)) {
          return this.formatLocal[index].indonesian;
        }
      }
    },
